...
/**
 * Returns unique foods (by lowercase name), most recent first.
 * Optionally filters by a query string.
 */
function getFoodHistory(query) {
  const entries = getEntries();
  const seen = new Map();

  // Traverse newest → oldest so the first occurrence per name is the latest
...
function showSuggestions(query) {
  const results = getFoodHistory(query);
...
function addEntry() {
  ...
  // Reset form, keep focus ready for next entry
  $qty.value = '';
  $qualifier.value = '';
  $foodName.value = '';
  $kcal.value = '';
  ...
  // If calendar is rendered, keep it fresh
...
function renderLogView() {
  ...
  for (const date of sortedDates) {
    ...
    // Label row with optional kcal total
    const label = document.createElement('div');
    label.className = 'date-label';
    label.textContent = formatDateLabel(date);

    const totalKcal = dayEntries.reduce((sum, e) => sum + (e.kcal || 0), 0);
...
function renderCalendar() {
  ...
  // Monday-first week: what weekday offset does the 1st have?
  ...
  // Show entries for the selected date (or a prompt)
  if (selectedDate) {
...
function renderCalEntries(dateStr) {
  const dayEntries = getEntries().filter(e => e.date === dateStr);
...
document.querySelectorAll('.toggle-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (view === 'calendar') {
      // Default selected day to today if no selection yet
